(function () {
  ...
  function categoryClass(rows, options) {
    ...
    const warnings = [];
    if (categories.length > colors.length) warnings.push(`Kategori berjumlah ${categories.length}; warna tambahan dibuat deterministik agar tidak memakai warna yang sama diam-diam.`);
    const colorFor = (index) => colors[index] || `hsl(${(index * 137.508) % 360} 58% 45%)`;
    const assignments = {};
    categories.forEach((category, index) => { assignments[category] = { label: category === "TANPA KATEGORI" ? "Tanpa kategori" : category, color: colorFor(index), index }; });
  ...
  function numericClass(rows, options, mode) {
    ...
    const warnings = [];
    if (!values.length) return { assignments: {}, legend: [], warnings: ["Tidak ada nilai numerik yang dapat dipetakan."], values: [] };
  ...
  function equalInterval(values, options, warnings) {
    ...
    if (min === max) {
      ...
      return { assignments: {}, legend: [{ label: formatNumber(min, options.numberFormat), color }], warnings: warnings.concat("Semua nilai sama; satu kelas digunakan."), values: values.map((row) => ({ rowId: row.rowId, matchedId: row.matchedId, classKey: "0", color })) };
    }
  ...
  function quantile(values, options, warnings) {
    ...
    if (classes < requested) warnings.push(`Nilai berulang membuat jumlah kelas efektif menjadi ${classes}. Nilai yang sama tetap berada pada kelas yang sama.`);
  ...
  function manualBreaks(values, options, warnings) {
    ...
    if (!raw.length || raw.some((value) => !Number.isFinite(value)) || raw.some((value, index) => index && value <= raw[index - 1])) throw new Error("Batas manual harus berupa angka yang naik dan tidak boleh duplikat.");
  ...
  function diverging(rows, options) {
    ...
    if (!below.length || !above.length) warnings.push("Semua nilai berada di satu sisi pusat; sisi lainnya tetap ditampilkan sebagai no-data.");
  ...
  function classify(rawRows, options = {}) {
    ...
    return { version: VERSION, paletteVersion: PALETTE_VERSION, method, options: Object.assign({ classes: options.classes || 5, palette: options.palette || (method === "categorical" ? "safe-default" : method === "diverging" ? "blue-orange" : "blue"), reverse: Boolean(options.reverse), numberFormat: options.numberFormat || "id-ID", noDataColor: options.noDataColor || "#D9E0E6" }, options), assignments: Object.fromEntries(result.values.map((item) => [item.matchedId, item])), legend: result.legend.concat(noData.length ? [{ label: "Tidak ada data", color: options.noDataColor || "#D9E0E6", key: "no-data" }] : []), warnings: result.warnings.concat(noData.length ? [`${noData.length} baris tidak diberi warna karena tidak memiliki nilai/kecocokan valid.`] : []), rows, noData: noData.map((row) => row.rowId), center: result.center == null ? null : result.center };
  }
